    ...
    if request.method == "OPTIONS":
        ...
        try:
            if(cursor.execute("SELECT * from inventory WHERE id = '"+(request.json[0].replace(" ","-")).lower()+"'").fetchone()):
                print("huh?")
                print("UPDATE inventory SET have ='"+request.json[1]+"', need ='"+request.json[2]+"', checkweekly ='"+request.json[3]+"' WHERE id='"+(request.json[0].replace(" ","-")).lower()+"'")
                ...
                print("INSERT INTO inventory(id, name, have, need, checkweekly, amountneededweekly, type, location) VALUES ('"+((request.json[0].replace(" ","-")).lower())+"','"+request.json[0].title()+"','"+request.json[1]+"','"+request.json[2]+"','"+request.json[3]+"','0.00','none','none')")
...
def getAllInventory():
    ...
    cursor = inv.cursor()
    responseList = []
    print("I don't think there's an item being requested")
