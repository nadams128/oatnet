		...
		if write:
			...
			if request.method == "POST":
				...
				try:
					...
					amountNeededWeekly = request.json[4]
					if (cursor.execute("SELECT * FROM inventory WHERE id=?",(id,)).fetchone()):
						cursor.execute("UPDATE inventory SET have=?, need=?, checkweekly=?, amountneededweekly=? WHERE id=?",(have, need, checkWeekly, amountNeededWeekly, id))
					else:
						cursor.execute("INSERT INTO inventory(id, name, have, need, checkweekly, amountneededweekly, type, location) VALUES (?,?,?,?,?,?,'none','none')",(id, name, have, need, checkWeekly, amountNeededWeekly))
